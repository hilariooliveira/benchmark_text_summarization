...
def select_sentences(sentences: list, summary_size: int, excluded_solutions: list = None) -> list:
    ...
    id_sentence = 's_' + str(sentence.id)

    s = pulp.LpVariable(id_sentence, lowBound=0, upBound=1, cat='Binary')

    dict_pulp_variables[id_sentence] = s

    sentence_size_constrant = (len([t for t in sentence.tokens if t.type == 'WORD' or t.type == 'NUM']) * s)

    for i in range(1, len(sentences)):
        sentence = sentences[i]
        id_sentence = 's_' + str(sentence.id)
        s = pulp.LpVariable(id_sentence, lowBound=0, upBound=1, cat='Binary')
        sentence_size = len([t for t in sentence.tokens if t.type == 'WORD' or t.type == 'NUM'])
